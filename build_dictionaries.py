...
class SynsetMap:
    ...
    def __init__(self, map_model: str, src_file: str):
        assert map_model in ['blc', 'macro'], 'Choose between blc|macro'
        if map_model == 'blc':
            self._build_blc_map(src_file)
        elif map_model == 'macro':
            self._build_macro_map(src_file)
        self.map_dict = OrderedDict()

    def _build_blc_map(self, src_file):
        pass

    def _build_macro_map(self, src_file):
        pass

    def map_synset(self, synset: str) -> Synset:
        return None
...
def main():
    parser = argparse.ArgumentParser(description="Build different dictionaries")
    build_1hop()
